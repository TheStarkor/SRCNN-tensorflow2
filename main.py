...
def train():
    ...
    checkpoint = ModelCheckpoint("SRCNN_check.h5", monitor='val_loss', verbose=2, save_best_only=True, save_weights_only=False, model='min')
    callbacks_list = [checkpoint]

    srcnn.fit(data, label, batch_size=128, validation_data=(val_data, val_label), callbacks=callbacks_list, shuffle=True, epochs=200, verbose=2)
