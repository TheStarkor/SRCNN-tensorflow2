...
def prepare_data(_path):
    ...
    data = numpy.zeros(
        (nums * Random_Crop, 1, Patch_size, Patch_size), dtype=numpy.double
    )
    label = numpy.zeros(
        (nums * Random_Crop, 1, label_size, label_size), dtype=numpy.double
    )
    ...
    for i in range(nums):
        ...
        for j in range(Random_Crop):
            lr_patch = lr_img[
                Points_x[j] : Points_x[j] + Patch_size,
            ...
            lr_patch = lr_patch.astype(float) / 255.0
            hr_patch = hr_patch.astype(float) / 255.0
